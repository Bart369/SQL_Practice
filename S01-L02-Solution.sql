...
FROM employee /* Do not shorten table name aliases SO Ileft this simply as employee */
...
INNER JOIN ( /* Subquery starts in the next line */
    ...
    ) AS department_employee_rel_id ON `department_employee_rel_id`.`employee_id` = `employee`.`id` 
-- Space between Multiple Joins
INNER JOIN department_employee_rel ON 1=1
    AND `department_employee_rel`.`id` = `department_employee_rel_id`.`max_id`  /* Abusing AND because it's awesome */
    AND `department_employee_rel`.`deleted_flag` = 0 
